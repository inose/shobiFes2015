...
function Awake ()
{
	if(!cameraTransform && Camera.main)	//カメラがここに設定されてない？かつカメラが存在する?.
		cameraTransform = Camera.main.transform;	//カメラを設定.
	if(!cameraTransform) {
		Debug.Log("Please assign a camera to the ThirdPersonCamera script.");
		enabled = false;	//カメラが無いのでこのスクリプトを停止.	
	}
	...
	if (controller)
	{
		var characterController : CharacterController = _target.GetComponent.<Collider>();	//Playerのコライダーを取得.
